...
void CTrail::TrailOn(_matrix _matWeapon)
{
	...
	for (_uint i = 0; i < 22; i += 2)
	{
		...
		_vector High = XMVector3TransformCoord(XMLoadFloat3(&(m_HighAndLow.vHigh)), _matWeapon);
		_vector Low = XMVector3TransformCoord(XMLoadFloat3(&(m_HighAndLow.vLow)), _matWeapon);
